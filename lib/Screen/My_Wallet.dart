...
import '../Helper/payment_getways.dart';
...
/*    new Future.delayed(Duration.zero, () {
  ...
  Future<Null> sendRequest(String? txnId, String payMethod, var amt) async {
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      String orderId =
          "wallet-refill-user-$CUR_USERID-${DateTime.now().millisecondsSinceEpoch}-${Random().nextInt(900) + 100}";
      print("$orderId >>>>>>>>>>>$CUR_USERID>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> orderId");
      try {
        var parameter = {
          USER_ID: CUR_USERID,
              ...
              child: Text(
                ...
                onPressed: () {
                  if (_formkey.currentState!.validate()) {
                    var options = {
                      'key': 'rzp_test_1spO8LVd5ENWfO', // Dummy test key
                ...
                /* print("fhfghfhfhfhfghf");
                final form = _formkey.currentState!;
                // sendRequest(response.paymentId, "RazorPay",amtC);
                _razorpay.on(Razorpay.EVENT_PAYMENT_SUCCESS, _handlePaymentSuccess);*/
  ...
  razorpayPayment(double price) async {
    ...
    if (contact != '' && email != '') {
      ...
      PhonePeHelper razorPay = PhonePeHelper("$amt", (result) async {
        if (result != "error") {
          print("??????????????????????????????????????????????????????????");
          // print(response.orderId);
          // print(response.paymentId);
          // print(response.signature);
          print("??????????????????????????????????????????????????????????");
          sendRequest("${transactionId}", "Phonepe", amt);
          ...
          // sendRequest("response.paymentId", "Phonepe", amt);
        }
      });

      razorPay.initPhonePeSdk();
  ...
  Widget listItem(int index) {
    final transaction = tranList[index];
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  "Success", // Static or from `transaction.status`
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    color: Colors.green,
                    fontSize: 13,
            ...
            // Middle Row: Icon + Received + Amount
            Row(
              children: [
                Container(
                  width: 36,
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            paypal = payment["paypal_payment_method"] == "1" ? true : true;
            paumoney =
