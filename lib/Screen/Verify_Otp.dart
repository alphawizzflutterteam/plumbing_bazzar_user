...
import 'package:Plumbingbazzar/Screen/Dashboard.dart';
import 'package:Plumbingbazzar/Screen/HomePage.dart';
import 'package:Plumbingbazzar/Screen/Set_Password.dart';
...
class _MobileOTPState extends State<VerifyOtp> with TickerProviderStateMixin {
  ...
  Future<void> getLoginUser() async {
    ...
    print("fffffffffffffffffffffff${response.body}");
  ...
  otpCheck() async {
    if (widget.otp.toString() == otp.toString()) {
      ...
      // setSnackbar(
      //     getTranslated(context, 'ENTEROTP') ?? "Please Enter Valid OTP!");
      Fluttertoast.showToast(
          msg: getTranslated(context, 'ENTEROTP') ?? "Please Enter Valid OTP!",
  ...
  mobText() {
    return Padding(
      ...
      child: Center(
        child: Column(
          children: [
            Text("${widget.mobileNumber}  Otp: ${widget.otp}",
                style: Theme.of(context).textTheme.subtitle1!.copyWith(
                    color: Theme.of(context).colorScheme.fontColor,
                    fontWeight: FontWeight.normal)),
          ],
...
class VerifyOtp1 extends StatefulWidget {
  final otp;
  final mobilenum;
  VerifyOtp1({Key? key, this.otp, this.mobilenum})
      : assert(otp != null),
        super(key: key);

  @override
  _MobileOTP1State createState() => _MobileOTP1State();
}

class _MobileOTP1State extends State<VerifyOtp1> with TickerProviderStateMixin {
  final dataKey = GlobalKey();
  String? password;
  dynamic? otp;
  bool isCodeSent = false;
  ...
  otpCheck() async {
    if (widget.otp.toString() == otp.toString()) {
      ...
      Fluttertoast.showToast(
          msg: getTranslated(context, 'OTPMSG')!,
          backgroundColor: colors.primary);
      settingsProvider.setPrefrence(MOBILE, widget.mobilenum!);
      // settingsProvider.setPrefrence(COUNTRY_CODE, widget.countryCode!);
      // if (widget.title == getTranslated(context, 'SEND_OTP_TITLE')) {
      Future.delayed(Duration(seconds: 2)).then((_) {
        Navigator.pushReplacement(
  ...
  mobText() {
    return Padding(
      ...
      child: Center(
        child: Column(
          children: [
            Text("${widget.mobilenum}  Otp: ${widget.otp}",
                style: Theme.of(context).textTheme.subtitle1!.copyWith(
                    color: Theme.of(context).colorScheme.fontColor,
                    fontWeight: FontWeight.normal)),
          ],
