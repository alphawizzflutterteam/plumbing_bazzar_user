...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  phonepayPayment() async {
    print("fghfgghghghgh ");
    SettingProvider settingsProvider =
        Provider.of<SettingProvider>(this.context, listen: false);
    print("Payment Email ${settingsProvider.email}");
    print("Payment Mobile ${settingsProvider.mobile}");
    String? contact = settingsProvider.mobile;

    String amt = ((double.parse(totalamount!.toString().replaceAll(",", ""))))
        .toStringAsFixed(2);

    if (contact != '') {
      print("fghfgghghghgh ddadasdasdasd");
      context.read<CartProvider>().setProgress(true);
      checkoutState!(() {});

      PhonePeHelper razorPay = PhonePeHelper("$amt", (result) async {
        print("fghfgghghghgh   dasdasdasdadsdasdasdasdasdasdasd ");
        if (result != "error") {
          print("fghfgghghghghresuyusyd ${result} ");
          placeOrder("${transactionId}");
          setState(() {});
        }
      });
  ...
  //       };
  //       Response response =
  //           await post(checkDeliveryApi, body: parameter, headers: headers)
  //               .timeout(Duration(seconds: timeOut));
  //       print(checkDeliveryApi.toString());
  //       print(parameter.toString());
