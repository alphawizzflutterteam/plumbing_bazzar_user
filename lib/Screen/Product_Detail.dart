...
import 'package:Plumbingbazzar/Helper/vimeoplayer.dart';
...
import 'package:Plumbingbazzar/Provider/ProductDetailProvider.dart';
import 'package:Plumbingbazzar/Provider/UserProvider.dart';
import 'package:Plumbingbazzar/Screen/Cart.dart';
import 'package:Plumbingbazzar/Screen/CompareList.dart';
import 'package:Plumbingbazzar/Screen/ProductList.dart';
import 'package:Plumbingbazzar/Screen/ReviewList.dart';
  ...
  Widget _slider() {
    ...
    return InkWell(
      onTap: () {
        ...
        //         videoType: null, //widget.model!.videType,
        //         from: false,
                            ...
                            child: Padding(
                                ...
                                child: Icon(
                                  !data.contains(widget.model!.id)
                                      ...
                                      : Icons.favorite ,color: (!data.contains(widget.model!.id)) ? colors.primary : Colors.red,
                                  size: 20,
                                )),
                            onTap: () async {
  ...
  _desc() {
    return widget.model!.desc!.isNotEmpty
        ? Padding(
            ...
            child: Html(data: sanitizeHtml(widget.model!.desc ?? "")))
        : Container();
  }
  ...
  _setFav(int index) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        var parameter = {USER_ID: curUserId, PRODUCT_ID: widget.model!.id};
        Response response =
              ...
              children: <Widget>[
                Column(
                  ...
                  children: [
                    ...
                    InfoCardRow(policy:widget.model?.cod_policy ?? "sdfsfsdf",  exchange: widget.model?.terms_n_condition ?? "sgdgd",),
        ...
        alignment: Alignment.topLeft,
              ...
              child: Row(
                ...
                children: [
                  ...
                  //     ),
                  //     onPressed: () {
                  //       String qty = ((int.parse(qtyController.text)) + 0).toString();
                  //       addToCart(qty, true);
                  //     },
                  //     child: Text(
...
class InfoCardRow extends StatefulWidget {
  final dynamic policy;
  final dynamic exchange;
  const InfoCardRow({super.key ,  required this.policy, required this.exchange});

  @override
  State<InfoCardRow> createState() => _InfoCardRowState();
}

class _InfoCardRowState extends State<InfoCardRow> {
  @override
  Widget build(BuildContext context) {
    return Container(
      color: Colors.white,
      child: Center(
        child: Row(
          ...
          children: [
            _buildCard(context, "Top Brand", Icons.verified, showDot: true ,widget.policy ),
            SizedBox(width: 16),
            _buildCard(context , "Exchange Policy", Icons.autorenew, widget.exchange ),
          ],
        ),
      ),
    );
 ...
 /* Widget _buildCard(String label, IconData icon, {bool showDot = false}) {
    return Stack(
      clipBehavior: Clip.none,
      children: [
        DottedBorder(
          ...
          dashPattern: [5, 3], // 5px line, 3px space
          child: Container(
            width: 150,
  ...
  Widget _buildCard(BuildContext context, String label, IconData icon,policy , {bool showDot = false}) {
...
class ProductDetail1 extends StatefulWidget {
  ...
  const ProductDetail1(
      {Key? key, required this.model, this.secPos, this.index, this.list})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => StateItem1();
}
...
class StateItem1 extends State<ProductDetail1> with TickerProviderStateMixin {
  int _curSlider = 0;
  final _pageController = PageController();
  final GlobalKey<ScaffoldState> _scaffoldKey = new GlobalKey<ScaffoldState>();
  List<int?> _selectedIndex = [];
                      ...
                      children: <Widget>[
                        Column(
                          ...
                          children: [
                            ...
                            InfoCardRow1(policy : widget.model?.cod_policy ?? "gdfgdgd" ,  exchange: widget.model?.terms_n_condition ?? "dfgfgd"),
                            /*  widget.model!.type == "variable_product"
                        ? Divider()
                        : Container(),*/
...
class AnimatedProgressBar1 extends AnimatedWidget {
  final Animation<double> animation;

  AnimatedProgressBar1({Key? key, required Animation<double> this.animation})
      : super(key: key, listenable: animation);
...
class _ControlsOverlay1 extends StatelessWidget {
  const _ControlsOverlay1({Key? key, this.controller}) : super(key: key);
...
class InfoCardRow1 extends StatefulWidget {
  final  dynamic policy;
  final  dynamic exchange;
  const InfoCardRow1({super.key ,  required this.policy ,  required this.exchange});

  @override
  State<InfoCardRow1> createState() => _InfoCardRow1State();
}

class _InfoCardRow1State extends State<InfoCardRow1> {
  @override
  Widget build(BuildContext context) {
    return Container(
      color: Colors.white,
      child: Center(
        child: Row(
          ...
          children: [
            _buildCard(context, "Top Brand", Icons.verified, showDot: true ,widget.policy ),
            SizedBox(width: 16),
            _buildCard(context , "Exchange Policy", Icons.autorenew,widget.policy),
          ],
        ),
      ),
    );
