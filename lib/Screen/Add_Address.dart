...
import 'package:Plumbingbazzar/Screen/Map.dart';
...
class StateAddress extends State<AddAddress> with TickerProviderStateMixin {
  ...
  StateSetter? areaState, cityState,StateState , tahsilState;
  ...
  List<States> StateSearchLIst = [];
  bool cityLoading = true,StateLoading = true ,areaLoading = true, tahsilLoading = true;
  ...
  User? selArea;
  User? selTahsil;
  int? selAreaPos = -1, selCityPos = -1,selStatePos = -1, selTahsilPos = -1;
  ...
  setUserName() {
    return Column(
      ...
      children: [
        ...
        Divider( // 👈 This is the new divider
 ...
 /* getAreaList() {
    ...
    return areaSearchList
        ...
        InkWell(
          onTap: () {
            print("fgdfgdgdfgdfgdfgdfgdfggfg${areaSearchList[index].city}");
            if (mounted) {
              setState(() {
                selAreaPos = index;
                selTahsil = null;
                ...
                String? areaIDValue = areaSearchList[selAreaPos!].id.toString();
                print("wwwwwwwwwwwwwwwww ${areaIDValue}");
          ...
          child: Container(
            ...
            child: Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text(
                areaSearchList[index].city?.isNotEmpty == true
...
/*  setStateValue1() {
 ...
 /* Future<void> getCities(dynamic id) async {
    print("cccccccccccccccccccccccccccccccc");
    try {
      String url = "${getCitiesApi}/$id";
 ...
 /* Future<void> getCities(dynamic id) async {
    print("Fetching cities for ID: $id");

    try {
      String url = "$getCitiesApi?state=$id";
      print('PrintData____${url}_____');
      ...
      if (!error) {
        var data = getdata["data"];
        cityList = (data as List).map((e) => User.fromJson(e)).toList();
        citySearchLIst.addAll(cityList);
      } else {
      ...
      // If in update mode, select the city based on existing address
      if (widget.update == true) {
        selCityPos = citySearchLIst.indexWhere(
              (f) => f.id == addressList[widget.index!].stateId,
        );
        if (selCityPos == -1) selCityPos = null;
      }
    } on TimeoutException {
      ...
      setSnackbar("Failed to fetch cities.");
    }
  }*/
  Future<void> getCities(dynamic id) async {
    print("Fetching cities for ID: $id");
    ...
    try {
      // String url = getCitiesApi; // Remove '?state=$id' from the URL
      // print('PrintData____${url}_____');
      ...
      print('PrintData;ljkjjjkjklj____${response.body}_____');
      var getdata = json.decode(response.body);
      bool error = getdata["error"];
      String? msg = getdata["message"];
  ...
  /*Future<void> getState() async {
    print("dgdfgddggfdfdggg");
    try {
      Response response = await post(getStateApi, headers: headers)
          .timeout(Duration(seconds: timeOut));
  ...
  Future<void> getArea(String? city, bool clear) async {
    ...
    try {
      ...
      print("id issss ${data}");
      Response response =
          await post(getCitiesApi, body: data, headers: headers)
              .timeout(Duration(seconds: timeOut));
      var getdata = json.decode(response.body);
      bool error = getdata["error"];
      String? msg = getdata["message"];
  ...
  setCountry() {
    return Column(
      ...
      children: [
        ...
        Padding(
          padding: const EdgeInsets.all(8.0),
          child: Divider( // 👈 This is the new divider
            color: Colors.grey.shade300,
            thickness: 1,
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          setLandmark(),
                              ...
                              Row(
                                children: [
                                  Expanded(child: setStateValue1()),
                                  SizedBox(width: 10),
                                  Expanded(child: setStateValue()),
                                  // Expanded(child: setArea()),
                                ],
                              ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                    selStatePos != null && selStatePos != -1
                                        ? StateSearchLIst[selStatePos!].name!
                                        : "",
                                    style: TextStyle(
                                        color: selStatePos != null
                                            ? Theme.of(context)
                                            .colorScheme
                                            .fontColor
                                            : Colors.grey)),
    ...
    return StateSearchLIst
        ...
        InkWell(
          onTap: () {
            ...
            cityID = StateSearchLIst[selStatePos!].id;
            getState();
            getCities(cityID);
          },
          child: Container(
  ...
  Future<void> getState() async {
    ...
    try {
      ...
      if (!error) {
        ...
        StateList =
            (data as List).map((data) => new States.fromJson(data)).toList();
      ...
      if (widget.update!) {
        ...
        if (selStatePos == -1) selStatePos = null;
      }
    } on TimeoutException catch (_) {
      setSnackbar(getTranslated(context, 'somethingMSg')!);
    }
