...
import 'package:Plumbingbazzar/Helper/Public%20Api/api.dart';
...
import 'package:Plumbingbazzar/Screen/Customer_Support.dart';
import 'package:Plumbingbazzar/Screen/MyTransactions.dart';
import 'package:Plumbingbazzar/Screen/ReferEarn.dart';
import 'package:Plumbingbazzar/Screen/SendOtp.dart';
import 'package:Plumbingbazzar/Screen/Setting.dart';
import 'package:Plumbingbazzar/Screen/Login.dart';
import 'package:Plumbingbazzar/Screen/policies_screen.dart';
...
class StateProfile extends State<MyProfile> with TickerProviderStateMixin {
  ...
  //         ],
  //       ),
  //       padding: const EdgeInsetsDirectional.only(start: 10.0, top: 12, bottom: 12, end: 10),
  //       child: Row(
  //         children: [
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Selector<UserProvider, String>(
                      selector: (_, provider) => provider.mob,
                      builder: (context, userMobile, child) {
                        print("vvvvvvvvvvvvvvvvvvvvv${userMobile}");
                        return userMobile != ""
                            ? Column(
                                mainAxisAlignment:
                    ...
                    Consumer<UserProvider>(builder: (context, userProvider, _) {
                      print("hhhhhhhhhhhhhhhhhhhhh${userProvider.curUserName}");
                      print("mmmmmmmmmmmmmmmmmmmmmm${userProvider.mob}");
                      return userProvider.mob == ""
                          ? Padding(
                              padding: const EdgeInsetsDirectional.only(top: 7),
                              child: InkWell(
...
/*  Future<void> setUpdateUser() async {
 ...
 /* _getDrawer()  async {
  ...
  Future<Widget> _getDrawer() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? curUserId = prefs.getString('CUR_USERID');
    return ListView(
      padding: EdgeInsets.zero,
...
/*  _getDivider() {
  ...
  //                     // title: getTranslated(context, 'SHIPPING'),
  //                     ),
  //               ));
  ...
  //                     // title: getTranslated(context, 'REFUND'),
  //                     ),
  //               ));
              ...
              if (title == getTranslated(context, 'MY_ORDERS_LBL')) {
                ...
                curUserId == null
                    ...
                    : Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => ManageAddress(home: true)));
              } else if (title == getTranslated(context, 'REFEREARN')) {
                Navigator.push(context,
                ...
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) => ShippingPolicy()));
              } else if (title == getTranslated(context, 'REFUND')) {
                Navigator.push(context,
                ...
                Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            Faqs(title: getTranslated(context, 'FAQS'))));
              } else if (title == getTranslated(context, 'CHANGE_THEME_LBL')) {
                openChangeThemeBottomSheet();
              } else if (title == getTranslated(context, 'LOGOUT')) {
                logOutDailog();
  ...
  Future<void> _openStoreListing() => _inAppReview.openStoreListing(
      appStoreId: 'com.Plumbingbazzar.main',
  ...
  Future<void> setProfilePic(File _image) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        request.fields[USER_ID] = curUserId!;
  ...
  Future<bool> validateAndSave(GlobalKey<FormState> key) async {
    ...
    if (form.validate()) {
      if (key == _changePwdKey) {
        ...
        setUpdateUser(
            curUserId!, "", "", nameController.text, emailController.text);
      }
      return true;
    }
    return false;
