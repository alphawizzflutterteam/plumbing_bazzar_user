// final String appName = 'MyTop10Store';
final String appName = 'Plumbingbazzar';
final String packageName = 'com.Plumbingbazzar.main';
final String androidLink = 'https://play.google.com/store/apps/details?id=';
final String iosPackage = 'com.Plumbingbazzar.main';
final String iosLink = 'your ios link here';
final String appStoreId = 'com.Plumbingbazzar.main';
final String deepLinkUrlPrefix = 'https://alpha.ecommerce.link';
final String deepLinkName = 'alpha.ecommerce.link';
final int timeOut = 50;
const int perPage = 10;
final String baseUrl = 'https://plumbing-new.alphawizzserver.com/app/v1/api/';
// final String baseUrl = 'https://plumbingbazzar.com/app/v1/api/';
// final String baseUrl = 'http://new-ecom.developmentalphawizz.com/app/v1/api/';
// final String baseUrl = 'https://mytop10store.com/app/v1/api/';
final String imageUrl = 'https://plumbingbazzar.com/';
final String jwtKey = "e1e421dceee9ea718ab764ea4fdef95629e7f415";
